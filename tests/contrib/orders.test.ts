...
describe('ConstructRepeatOrderTest', () => {
    const assertBuilder = (expectedJson: any, builder: OrderBuilder) => {
        expect(JSON.stringify(builder.build(), null, 4)).toBe(
            JSON.stringify(expectedJson, null, 4)
        );

        const validateSyntax = (code: string, globalz: any) => {
            const splitCode = code.split('\n');
            const lineFormat = (lineNum: number, line: string) => 
                ` ${String(lineNum + 1).padStart(splitCode.length.toString().length)}   ${line}`;
            
            console.log('Generated code:');
            console.log();
            console.log(splitCode.map((line, i) => lineFormat(i, line)).join('\n'));

            try {
                // In TypeScript/Jest, we'll use eval instead of exec
                eval(code);
            } catch (e) {
                console.log();
                console.log(e);
                throw new Error('Syntax error from generated code');
            }
        };

        // With a variable name, validate the syntax and expect the output
        const code = codeForBuilder(builder, 'testBuilder');
        const globalz: any = {};
        validateSyntax(code, globalz);
        expect(JSON.stringify(globalz.testBuilder.build(), null, 4)).toBe(
            JSON.stringify(expectedJson, null, 4)
        );

        // With no variable name, just validate the syntax
        const codeWithoutVar = codeForBuilder(builder);
        validateSyntax(codeWithoutVar, {});
    };

    test('market_equity_order', () => {
        ...
        const repeatOrder = constructRepeatOrder(historicalOrder);
    ...
    test('missing_orderStrategyType', () => {
        const historicalOrder = {
            "session": "NORMAL",
            "duration": "DAY",
            "orderType": "MARKET",
        ...
        expect(() => constructRepeatOrder(historicalOrder))
            .toThrow('historical order is missing orderStrategyType');
    });

    test('unknown_orderLegType', () => {
        const historicalOrder = {
            "session": "NORMAL",
            "duration": "DAY",
            "orderType": "MARKET",
        ...
        expect(() => constructRepeatOrder(historicalOrder))
            .toThrow('unknown orderLegType');
    });

    test('unknown_orderLegType_codegen', () => {
        const historicalOrder = {
            "session": "NORMAL",
            "duration": "DAY",
            "orderType": "MARKET",
        ...
        const repeatOrder = constructRepeatOrder(historicalOrder);
        (repeatOrder as any)._orderLegCollection[0].instrument._assetType = 'BOGUS';

        expect(() => codeForBuilder(repeatOrder))
            .toThrow('unknown leg asset type');
    });

    test('limit_options_order', () => {
        const historicalOrder = {
            "session": "NORMAL",
            "duration": "DAY",
            "orderType": "LIMIT",
